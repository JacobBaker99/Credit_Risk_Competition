...
and train_base.csv. This is the default Deision tree from sklearn with 100 trees, baseline.
'''
...
def test(X_scaled, y):
    ...
    for train_index, test_index in skf.split(X_scaled, y):
        ...
        rf_model = ensemble.RandomForestClassifier(criterion='entropy', n_estimators=TREES,
            random_state=SEED)
        rf_model.fit(X_train, y_train)
        print(f'\tRF_entropy: {accuracy_score(y_test, rf_model.predict(X_test)) * 100:.2f}%')

    rf_scores = cross_val_score(ensemble.RandomForestClassifier(criterion='entropy',
        n_estimators=TREES, random_state=SEED), X_scaled, y, cv=skf, scoring="accuracy")
