...
'''
multinomial naive bayes for Project One

Alpha is default here (1.0).

This module is used to conduct multinomial naive bayes on dev.csv
'''
...
def prepData():
    '''
    Prepare the data from the csv for k-fold and multinomial naive bayes
    '''
...
def analyze(encoded_features, all_feature_names, labels):
    '''
    Execute k-fold and multinomial naive bayes
    '''
    hyper = [3270, 5, 1]
    scaler = MinMaxScaler()
    ...
    skf = StratifiedKFold(n_splits=hyper[1],
                            shuffle=True, random_state=hyper[0])
    for train_index, test_index in skf.split(X_scaled, labels):
        X_train, X_test = X_scaled[train_index], X_scaled[test_index]
        y_train, y_test = labels.iloc[train_index], labels.iloc[test_index]

        mnb_model = MultinomialNB(alpha=hyper[2])
        mnb_model.fit(X_train, y_train)
        print(f'\tMNB: {accuracy_score(y_test, mnb_model.predict(X_test)) * 100:.2f}%')


    mnb_scores = cross_val_score(MultinomialNB(alpha=hyper[2]),
                    X_scaled, labels, cv=skf, scoring="accuracy")
    print(f'Multinomial Naive Bayes Scores: {mnb_scores.mean():.4f} ± {mnb_scores.std():.4f}')
